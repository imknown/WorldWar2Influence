package com.dengdeng123.blowupplane.base.uncaught;
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
	...
	// private BeginApplication mApplication;
	...
	private void showLongTimeUncaughtMessageToast() {
		Toast longToast = new Toast(mContext);
		...
		TextView message_txtv = (TextView) viewLayout.findViewById(R.id.message_txtv);

		longToast.setView(viewLayout);

		message_txtv.setText(R.string.unknown_exception);
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			public void run() {
				...
				showLongTimeUncaughtMessageToast();

				// BeginApplication.showLongTimeMessageToast("很抱歉, 程序出現了問題.\n日誌保存中, 請及時聯繫我們.");
				// Toast.makeText(mContext, "很抱歉, 程序出現了問題.\n日誌保存中, 請及時聯繫我們.", Toast.LENGTH_LONG).show();
				Looper.loop();
			}
		}.start();
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				String path = Environment.getExternalStorageDirectory() + "/DengdengBlowUpPlane/CrashLog/";
